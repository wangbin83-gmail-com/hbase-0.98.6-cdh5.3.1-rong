...
public class DefaultMobCompactor extends DefaultCompactor {
  ...
  protected boolean performCompaction(FileDetails fd, InternalScanner scanner, CellSink writer,
      long smallestReadPoint, CompactionThroughputController throughputController) throws IOException {
    int bytesWritten = 0;
    ...
    try {
      ...
      if (mobFileWriter != null) {
        appendMetadataAndCloseWriter(mobFileWriter, fd, false); // false for major compaction on mob file
      }
    }
    if(mobFileWriter!=null) {
      if (mobCells > 0) {
